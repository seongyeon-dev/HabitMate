...
if (challenge) {
  ...
  if (today > endDate) {
    ...
    /* 종료 리포트 이동 */
    location.href = "challenge-report.html?id=" + challenge.id;
  }
}
...
if (challenge) {
  ...
  if (challengeMeta[1]) {
    if (challenge.mode === "solo") {
      ...
      challengeMeta[1].innerText = "함께 도전";
    }
  }
  ...
  /* 진행률 */
  if (progressText) {
    progressText.innerText = (challenge.progress || 0) + "%";
  }

  if (progressBar) {
    progressBar.style.width = (challenge.progress || 0) + "%";
  }

  if (progressDescription) {
    progressDescription.innerText =
      (challenge.successDays || 0) + "일 성공 / 총 " + challenge.period + "일";
  }
  ...
  if (challenge.customGoal) {
    ...
    goalText = challenge.goalValue + " " + getGoalUnit(challenge.goalUnit);
  }
...
if (checkButton) {
  checkButton.addEventListener("click", function () {
    ...
    /* 글 인증 확인 */
    if (type === "text") {
      if (!checkText || checkText.value.trim() === "") {
        alert("인증 내용을 입력해주세요.");
        return;
      }
    }
    ...
    /* 인증 기록 배열 */
    if (!Array.isArray(challenge.checkRecords)) {
      challenge.checkRecords = [];
    }

    /* 오늘 인증 기록 확인 */
    const todayRecord = challenge.checkRecords.find(function (record) {
    ...
    /* 인증 기록 추가 */
    challenge.checkRecords.push(checkRecord);

    /* 성공 일수 계산 */
    challenge.successDays = challenge.checkRecords.filter(function (record) {
    ...
    /* 마지막 인증 날짜 */
    challenge.lastCheckDate = todayText;
    ...
    if (progressText) {
      progressText.innerText = challenge.progress + "%";
    }
    ...
    if (progressDescription) {
      progressDescription.innerText =
        challenge.successDays + "일 성공 / 총 " + challenge.period + "일";
    }
...
const inviteButton = document.querySelector(".invite-member");
